class PlotCreator{
     ...
     */
    constructor(dom_container, nodes_data, links_data, plot_width, plot_height, first_column, last_column, settings = {}){
        //  REQUIRED ARGUMENTS
        ...
        Object.defineProperties(this,{
            vertical_gap_between_nodes: {value:default_settings['vertical_gap_between_nodes']},
            node_percent_of_column_width: {value:default_settings['node_percent_of_column_width']},
            show_column_lines: {value:default_settings['show_column_lines'],
                writable: true},
            show_column_names: {value:default_settings['show_column_names'],
                writable: true},
            show_links_out_of_range: {value:default_settings['show_links_out_of_range']},
            node_move_y: {value:default_settings['node_move_y'],
                writable: true},
            linear_gradient_links: {value:default_settings['linear_gradient_links'],
                writable: true},
            link_min_arc: {value:default_settings['link_min_arc']},
            link_arc_iterated_increase: {value:default_settings['link_arc_iterated_increase']},
            plot_background_color: {value:default_settings['plot_background_color']},
            default_links_color: {value:default_settings['default_links_color']},
            default_nodes_color: {value:default_settings['default_nodes_color']},
            default_links_opacity: {value:default_settings['default_links_opacity']},
            default_gradient_links_opacity: {value:default_settings['default_gradient_links_opacity']},
            default_sublinks_color: {value:default_settings['default_sublinks_color']},
            default_sublinks_opacity: {value:default_settings['default_sublinks_opacity']},
            label_colors_object: {value:default_settings['label_colors_object']},
            column_names: {value:default_settings['column_names']},
            start_node_count_from: {value:default_settings['start_node_count_from']},
            start_column_count_from: {value:default_settings['start_column_count_from']},
            on_node_click_function: {value:default_settings['on_node_click_function']},
            on_link_click_function: {value:default_settings['on_link_click_function']},
            on_node_hover_function: {value:default_settings['on_node_hover_function']},
            on_link_hover_function: {value:default_settings['on_link_hover_function']},
            hover_node_cursor: {value:default_settings['hover_node_cursor']},
            hover_link_cursor: {value:default_settings['hover_link_cursor']},
            grabbing_node_cursor: {value:default_settings['grabbing_node_cursor']},

        })
        this.node_move_x = false
        ...
        this.lines_style_object = {stroke: 'red', 'stroke-width': 2, 'stroke-opacity': 0.35, 'stroke-dasharray': '5,5'}
        ...
        this.column_names_style_object = {'font-size':'15px', color:'red', opacity:0.50,'font-weight':'bold'}
        if(settings.hasOwnProperty('column_names_style_object')){
            Object.assign(this.column_names_style_object,settings['column_names_style_object'])                             // writable
        }
        ...
        this.#sortLinks()
        this.#prepareData()
        this.last_column_width = this.plot_width / (this.last_column-this.first_column) // width of column in last plot, remembered to calculate shift or something, can't remember
        this.sankey_plot = this.#setNewPlot()
    }
    #sortLinks(){
        //sort by node order from
        this.links_data_structure.sort((a,b)=>{return a['from']['node']-b['from']['node']})
        // sort by columns
        this.links_data_structure.sort((a,b)=>{return a['from']['column']-b['from']['column']})
        // sort by node order to
        this.links_data_structure.sort((a,b)=>{return a['to']['node']-b['to']['node']})
        // sort by link length
        this.links_data_structure.sort((a,b)=>{return (a['to']['column']-a['from']['column'])-(b['to']['column']-b['from']['column'])})
    ...
    #preprocessLinksData(){ // change or add linktypes and orientation info
    ...
    #prepareData(){
        ...
        for (let i in this.nodes_data_structure){
            ...
            for (let j in this.nodes_data_structure[i]){
                ...
                let height = 0
                node['right_side_sum']>node['left_side_sum'] ? height=node['right_side_sum'] : height=node['left_side_sum']
                node['height'] = height
                if(height>max){
                    max = height
                }
        ...
        let max_column_sum = Math.max.apply(null,col_sums)
        this.node_height_unit = (1 - this.vertical_gap_between_nodes) * this.plot_height / max_column_sum

        //////////////////////////////////////////////////////////////////////////////////////// SETTING (EXACT) NODES Y COORD AND (EXACT) NODES HEIGHT
        for (let i in this.nodes_data_structure){
            let gaps = this.nodes_data_structure[i].length + 1
            let gap_height = (this.vertical_gap_between_nodes*this.plot_height/this.node_height_unit +
                (max_column_sum - col_sums[i]))/gaps
        ...
        for (let column_index in this.nodes_data_structure){
            for (let node_index in this.nodes_data_structure[column_index]){
                ...
                let left_side_links_normal = []
                let right_side_links_normal = []
                let left_side_links_special = []
                let right_side_links_special = []
                for (let link_index in node['links_references']){
                    let link = this.links_data_structure[node['links_references'][link_index]['link_id']]
                    switch (link['link_type']){
                        case ('LL'):
                            if (node['links_references'][link_index]['second_node_column']>column_index){left_side_links_special.push(link)}
                            else{left_side_links_normal.push(link)}
                            break
                        case ('LR'):
                            if (node['links_references'][link_index]['second_node_column']>column_index){left_side_links_special.push(link)}
                            else if (node['links_references'][link_index]['second_node_column']==column_index){
                                left_side_links_special.push(link)
                                right_side_links_special.push(link)
                            }
                            else {right_side_links_special.push(link)}
                            break
                        case ('RR'):
                            if (node['links_references'][link_index]['second_node_column']>column_index){right_side_links_normal.push(link)}
                            else {right_side_links_special.push(link)}
                            break
                        case ('RL'):
                        case undefined:
                ...
                // SPECIAL
                left_side_links_special.sort((a,b)=>{return b['to']['node']-a['to']['node']}) // ;)
                left_side_links_special.sort((a,b)=>{return b['to']['column']-a['to']['column']}) // ;)
                let node_height = node['height']
                let left_side_curve = -this.link_arc_iterated_increase + this.link_min_arc
                let left_side_occupied_height_special = 0
                for (let i in left_side_links_special){
                    let special_link = left_side_links_special[left_side_links_special.length - 1 - i] // reversed because of the arcs
                    ...
                    special_link['left_side_curve'] = left_side_curve+this.link_arc_iterated_increase
                    left_side_curve += link_height+this.link_arc_iterated_increase

                }
                right_side_links_special.sort((a,b)=>{return b['from']['node']-a['from']['node']}) //?
                right_side_links_special.sort((a,b)=>{return a['from']['column']-b['from']['column']}) //?
                let right_side_curve = -this.link_arc_iterated_increase + this.link_min_arc
                let right_side_occupied_height_special = 0
                for (let i in right_side_links_special){
                    let special_link = right_side_links_special[right_side_links_special.length - 1 - i] // reversed because of the arcs
                    let link_height = special_link['value'] * this.node_height_unit
                    ...
                    special_link['right_side_curve'] = right_side_curve+this.link_arc_iterated_increase
    ...
    changeColumnRange(first_column, last_column){
        if (first_column<this.start_column_count_from || last_column > this.nodes_data_structure.length + this.start_column_count_from || first_column>=last_column){
...
class SankeyPlot {
    constructor(dom_container, nodes_data, links_data, plot_width, plot_height, nodes_width_percent,
                first_column, last_column, node_height_unit, longest_link_length,last_column_width,show_column_lines=true,
                show_column_names=true, node_move_x = false, node_move_y = true,
                linear_gradient_links=true, link_arc_iterated_increase=5,label_colors_object={},
                plot_background_color='#f5ecec', default_links_color='blue', default_links_opacity=0.25,
                default_gradient_links_opacity=0.43,default_sublinks_color='red',default_sublinks_opacity=0.8,
                line_definition_object = {stroke: 'red',
                    'stroke-width': 2, 'stroke-opacity': 0.35, 'stroke-dasharray': '5,5'},
                column_names = undefined,
                column_names_style = {'font-size':'15px',color:'red',
                    opacity:0.50,'font-weight':'bold'},
                default_nodes_color = 'grey', start_node_count_from = 0,start_column_count_from = 0,
                show_links_out_of_range = true,
                on_node_click_function = undefined, on_link_click_function = undefined,
                on_node_hover_function = undefined, on_link_hover_function = undefined,
                hover_node_cursor = 'pointer', hover_link_cursor = 'help', grabbing_node_cursor = 'grabbing'
                ){
        ...
        if (show_column_lines){this.#createLines()}
        if (show_column_names){this.#createColumnNames()}
    ...
    #createDomNode(){
        ...
        set_attributes(this.dom_node, {width: this.plot_width, height: this.plot_height,
            id: 'sankey_field', viewBox: `0 0 ${this.plot_width} ${this.plot_height}`,})
        this.dom_node.style.backgroundColor = this.plot_background_color
        this.dom_container.appendChild(this.dom_node)
    }
    #getNodeInfo(e){
        let selected_node = this.nodes[e.target.getAttribute('column')][e.target.getAttribute('position')]
        ...
        for (let i in node_data['links_references']){
            ...
            if(node_column>second_node_col){
                if(!link.hasOwnProperty('link_type')){number_of_links_left+=1}
                else if (link['link_type'][1]==='L'){number_of_links_left+=1}
                else if (link['link_type'][1]==='R'){number_of_links_right+=1}}
            else if(node_column<second_node_col){
                if(!link.hasOwnProperty('link_type')){number_of_links_right+=1}
                else if (link['link_type'][0]==='L'){number_of_links_left+=1}
                else if (link['link_type'][0]==='R'){number_of_links_right+=1}}
        }
            //  end

        if (this.column_names){node_column = this.column_names[node_column]}
        else{node_column+=this.start_column_count_from}
    ...
    #getLinkInfo(e){
        let link_id = e.target.getAttribute('links_array_id')
        ...
        // check if sublink
        let sublink_index = e.target.getAttribute('sublink_index')
        if(sublink_index){                    
    ...
    #addEventListeners(){
        ...
        this.dom_node.addEventListener('click', (e)=>{
            if(e.target.getAttribute('identifier') === 'SankeyNode'){
                if(this.on_node_click_function){
                    ...
                    let selected_node = this.nodes[node_div.getAttribute('column')][node_div.getAttribute('position')]
                    let node_data = this.nodes_data_structure[selected_node.column][selected_node.position]['properties_object']
                    // execute callback function
                    this.on_node_click_function(this.#getNodeInfo(e), node_data, node_div,e)
                }
            }
            if(e.target.getAttribute('identifier') === 'SankeyLink'){
                if(this.on_link_click_function){
                    let link_div = e.target
                    let link_data = this.links_data[e.target.getAttribute('links_array_id')]
                    // execute callback function
        ...
        this.dom_node.addEventListener('pointerdown', (e) => {
            if(e.target.getAttribute('identifier') === 'SankeyNode'){
                ...
                this.selected_node = this.nodes[e.target.getAttribute('column')][e.target.getAttribute('position')]
        ...
        this.dom_node.addEventListener('pointerover', (e)=>{
            ...
            if(e.target.getAttribute('identifier') === 'SankeyNode') {
                if(!this.selected_node) {
                    ...
                    this.mouseovered_node = this.nodes[e.target.getAttribute('column')][e.target.getAttribute('position')]
                    
                    let text_content = undefined
                    // execute callback function
                    if(this.on_node_hover_function){
                        let div_ = e.target
                        let snode_ = this.nodes[div_.getAttribute('column')][div_.getAttribute('position')]
                        let node_d_ = this.nodes_data_structure[snode_.column][snode_.position]['properties_object']
                    ...
                    // position info
                    let potential_y_coord = this.mouseovered_node.current_y - node_info_div.offsetHeight + this.dom_container.getBoundingClientRect().top - 5
                    let potential_x_coord = this.mouseovered_node.current_x +this.dom_container.getBoundingClientRect().left+ (this.mouseovered_node.width - node_info_div.offsetWidth) / 2
                    potential_y_coord = potential_y_coord > 0 ? potential_y_coord : 0
                    potential_x_coord = potential_x_coord > 0 ? potential_x_coord : 0
                    if (potential_y_coord===0){potential_x_coord+= node_info_div.offsetWidth}
            ...
            // if link hovered
                ...
                if(!this.selected_node) {
                    ...
                    // create element
                    let link_info_div = document.createElement('div')
                    ...
                    let potential_y_coord = e.clientY - link_info_div.offsetHeight - 15
                    let potential_x_coord = e.clientX + 15
                    link_info_div.style.top = `${potential_y_coord>0?potential_y_coord:0}px`
                    link_info_div.style.left = `${potential_x_coord>0?potential_x_coord:0}px`
    ...
    #createColumnNames(){
        for (let i=0;i<this.last_column-this.first_column;i++) {
            ...
            if (this.column_names){number.textContent = this.column_names[i+this.first_column]}
            else {number.textContent = i + this.first_column + this.start_column_count_from}
            set_attributes(number, {
                x: this.column_width / 2 + i * this.column_width, y: 10, 'text-anchor': 'middle',
    ...
    #createNodes(){
        ...
        for (let column_index=this.first_column;column_index<this.last_column;column_index++){
            for (let node_index in this.nodes_data_structure[column_index]){
                for(let link_index in this.nodes_data_structure[column_index][node_index]['links_references']){
                    ...
                    if (link_ref['second_node_column']<this.first_column
        ...
        if(this.show_links_out_of_range){
            ...
            let check_range = number_of_columns - this.last_column > this.first_column ?
                [this.first_column-this.longest_link_length>0 ?
                     this.first_column-this.longest_link_length : 0, this.first_column]
                 : 
                [this.last_column, this.last_column+this.longest_link_length < number_of_columns ?
        ...
        for (let column_index=this.first_column;column_index<this.last_column;column_index++){
            ...
            for (let node_index=0; node_index<this.nodes_data_structure[column_index].length; node_index++){
                ...
                // LABEL
                let label = ""
                if(node['properties_object'].hasOwnProperty('label')){label = node['properties_object']['label']}
                // COLOR
                let color = this.default_nodes_color
                if(this.label_colors_object.hasOwnProperty(label)){color = this.label_colors_object[label]}
                if(node['properties_object'].hasOwnProperty('color')){color = node['properties_object']['color']}
                // WIDTH
                let width = 0
                if(node['properties_object'].hasOwnProperty('width')){width = this.column_width*parseFloat(node['properties_object']['width'])}
                else {width = this.column_width*this.nodes_width_percent}

                this.nodes[column_index][node_index] = new
                SankeyNode(
        ...
        for (let i in out_of_column_range_nodes){
            ...
            this.nodes[column][position] = new
            SankeyNode(
    ...
    #createLinks(){
        this.links_to_create_indexes.sort((a, b)=>{return a-b})
        for (let i in this.links_to_create_indexes){
            let link = this.links_data[this.links_to_create_indexes[i]]
            let link_type = 'RL'
            if (link.hasOwnProperty('link_type')){link_type = link['link_type']}
            ...
            if (link.hasOwnProperty('sublinks')){
                for (let sublink_index in link['sublinks']){
                    ...
                    switch (link_type){
                        case ('RL'):
                            new SankeyLinkRL(start_node, end_node, height,this, this.links_to_create_indexes[i], link_color, [sublink_data,sublink_index])
                            break
                        case ('LR'):
                            new SankeyLinkLR(start_node, end_node, height,this, this.links_to_create_indexes[i], link_color, [sublink_data,sublink_index])
                            break
                        case ('LL'):
                            new SankeyLinkLL(start_node, end_node, height,this, this.links_to_create_indexes[i], link_color, [sublink_data,sublink_index])
                            break
                        case ('RR'):
                            new SankeyLinkRR(start_node,end_node, height, this, this.links_to_create_indexes[i], link_color, [sublink_data,sublink_index])
                            break
                        default:
                            throw new Error('NOT A VALID LINK TYPE')
                    }
...
class SankeyNode{
    constructor(x, y, width, height,sankey_plot_object, column, position, color = 'blue', label ='',
                properties_object={}) {
        this.original_x = x
        this.original_y = y
        this.current_x = x
    ...
    #createDomElement(){
        ...
        */
        ...
        set_attributes(this.dom_rectangle, {'width': `${this.width}px`, 'height':this.height, 'fill':this.original_color,
            'x': 0, 'y':0, 'identifier': 'SankeyNode','column': this.column,
            'position': this.position})
    }
    move(new_x, new_y){
...
class SankeyLink{
    constructor(node1, node2, height, sankey_plot_object,links_array_index, color=undefined, sublink_data=undefined){
        this.node1 = node1
        this.node2 = node2
        this.height = height
        this.sankey_plot_object = sankey_plot_object
    ...
    updateD(){
        this.dom_node.setAttribute('d', this._calculateD(
    ...
    #linearGradient(){
        if (this.sankey_plot_object.linear_gradient_links && !this.fill && !this.sublink_data){
            let color1 = this.node1.original_color
            let color2 = this.node2.original_color
    ...
    _createDomElement(){
        ...
        if (this.sublink_data){this.dom_node.setAttribute('sublink_index',this.sublink_index)}
        this.sankey_plot_object.dom_node.appendChild(this.dom_node)
    }
}
